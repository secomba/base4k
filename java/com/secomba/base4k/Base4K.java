...
public class Base4K 
{
	...
	private static final int BASE1_START     = 0x05000;
	...
	public static byte[] decode(String encoded) throws DecodingFailedException
	{
		int code = 0;
		...
		for(int i=0;i<intCollector.size();i++) {
			if(intCollector.get(i)>=BASE1_START) {
				intCollector.set(i, intCollector.get(i) - BASE1_START);
			} else {
